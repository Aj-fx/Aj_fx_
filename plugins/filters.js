...
if(pattern.test(message.message)){
    ...
    Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {   
        ...
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '918281440156@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./kaztroser/Kaztroserv2/Hi.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: Config.SAID, contextInfo: { forwardingScore: 5, isForwarded: true }, quoted : message.data, ptt: true})
        }
        if (!!message.mention && message.mention[0] == Config.MENTION) {
...
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
   await message.client.sendMessage(message.jid, fs.readFileSync('./kaztroser/Kaztroser/' + a + '.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted: message.data, ptt: false})
}
});
}
